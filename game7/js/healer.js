...
function updateHealers(ships, dt) {
  for (const h of ships) {
    ...
    if (h.hp <= 0) {
      ...
      // stopped dead mid-air would read as having been shot.
    ...
    if (!t || t.dead || t.hp >= t.maxHp) t = pickPatient(ships);
...
// The damaged ship farthest from the planet's centre. The medic is never a
// patient itself.
function pickPatient(ships) {
