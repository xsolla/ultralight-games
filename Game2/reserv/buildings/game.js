...
// Skyline — decorative scrolling buildings flanking the road (purely cosmetic)
const BUILDING_PERIOD         = 1.1;   // world-Z spacing between building slots, per side
const BUILDING_SIDE_PHASE     = 0.55;  // world-Z offset of right-side slots vs left (desync skylines)
const BUILDING_FAR_WZ         = 14;    // furthest relZ a building is drawn (atmospheric cutoff)
const BUILDING_NEAR_CULL      = 0.6;   // nearest relZ before a building is skipped (passed camera)
const BUILDING_GAP_MIN        = 65;    // world-x gap beyond the road edge (road half-width = 300)
const BUILDING_GAP_MAX        = 240;   // (min..max gives a randomized ± setback per building)
const BUILDING_W_MIN          = 55;    // world-x width (narrowest building)
const BUILDING_W_MAX          = 475;   // widest building — 2.5× the former max for more variety
const BUILDING_DEPTH          = 0.8;   // world-Z thickness of each building box (side + top faces)
const BUILDING_H_MIN          = 0.45;  // height as fraction of camera height (1.0 ≈ full canvas)
const BUILDING_H_MAX          = 1.7;
const BUILDING_SKIP_CHANCE    = 0.16;  // fraction of slots left empty → ragged skyline
const BUILDING_FADE_MIN_A     = 0.12;  // min alpha for the most distant buildings
const BUILDING_WINDOW_NEAR_WZ = 5.0;   // only buildings nearer than this show lit windows
const BUILDING_WINDOW_ROWS    = 5;     // window grid density (rows/cols on a near face)
...
// Audio
const SFX_VOLUME    = 0.6;    // 0–1 sound-effects volume
const BGM_VOLUME    = 0.45;   // 0–1 background-music volume
const MUSIC_FADE_MS = 3000;   // ms to fade music to silence when returning to the title screen
...
const COLORS = {
  ...
  buildingTop:    '#2c2c54',                 // skyline roof face — lightest (sky-lit)
  buildingFront:  '#20203c',                 // front face — mid (clearly above gutter)
  buildingSide:   '#14142a',                 // inner side face — darkest, but > gutter so it reads
  buildingEdge:   'rgba(60,64,110,0.55)',    // subtle silhouette outline vs gutter
  buildingRim:    'rgba(120,140,235,0.55)',  // neon rim light on the near vertical corner
  buildingWindow: 'rgba(126,216,240,0.6)',   // lit window specks (cyan, matches playerFill)
};
...
function spawnComet() {
  ...
  if (Game.state !== 'title') playSound('comet');   // keep the title attract screen quiet
}
...
// ═══════════════════════════════════════════════════════════════════
//  DRAW: Skyline — scrolling building silhouettes flanking the road
//
//  Buildings are procedural (no state): each Z-slot's size is a pure
//  function of its index via buildingHash, so they never flicker as
//  they approach.  Placed at |worldX| ≥ ROAD_HALF_W + gap, they project
//  strictly outside the road edge at every depth, so they can never
//  occlude gameplay.  relZ = absZ − cameraZ drives the scroll, so they
//  flow at the current movement speed and speed up as the game ramps.
//
//  Projection: screenX = VP_X + worldX/relZ; ground y = wz2y(relZ);
//  a point at height fraction h sits at HORIZON_Y + (1−h)·PERSP_K/relZ.
// ═══════════════════════════════════════════════════════════════════
function drawBuildings(ctx) {
  const cz = Game.cameraZ;
  const lerp = (a, b, t) => a + (b - a) * t;

  // Gather all visible buildings from both sides so we can paint far→near.
  const list = [];
  for (const sideSign of [-1, 1]) {
    ...
    for (let k = kMin; k <= kMax; k++) {
      if (buildingHash(k, saltBase + 1) < BUILDING_SKIP_CHANCE) continue;   // skyline gap
      ...
      list.push({ k, sideSign, saltBase, relZ, gap, width, height });
    }
  }
  list.sort((a, b) => b.relZ - a.relZ);   // far first → nearer buildings overlap

  ctx.save();
  for (const b of list) {
    const relZf = b.relZ;                    // near (front) face depth
    const relZb = b.relZ + BUILDING_DEPTH;    // far (back) face depth
    const innerX = b.sideSign * (ROAD_HALF_W + b.gap);            // edge toward the road
    const outerX = b.sideSign * (ROAD_HALF_W + b.gap + b.width);  // edge away from the road

    // Project the 8 box corners.  sx(x, rz) = VP_X + x/rz; ground = wz2y(rz);
    // a point at height fraction h sits at HORIZON_Y + (1−h)·PERSP_K/rz.
    const groundF = wz2y(relZf), groundB = wz2y(relZb);
    const topF    = HORIZON_Y + (1 - b.height) * PERSP_K / relZf;
    const topB    = HORIZON_Y + (1 - b.height) * PERSP_K / relZb;
    const fInner = VP_X + innerX / relZf, fOuter = VP_X + outerX / relZf;   // front face
    const bInner = VP_X + innerX / relZb, bOuter = VP_X + outerX / relZb;   // back face

    // Atmospheric fade: distant buildings melt into the background
    const t    = (b.relZ - BUILDING_NEAR_CULL) / (BUILDING_FAR_WZ - BUILDING_NEAR_CULL);
    const fade = lerp(1, BUILDING_FADE_MIN_A, Math.max(0, Math.min(1, t)));
    ctx.globalAlpha = fade;

    // Receding faces first (top + inner side), then the front face on top.
    // Top (roof) face — lightest, slanting back toward the vanishing point.
    ...
    // Inner side face (toward the road) — darkest, gives the box its volume.
    ...
    // Front face — axis-aligned screen rectangle (both corners at relZf).
    const faceL = Math.min(fInner, fOuter);
    const faceW = Math.abs(fOuter - fInner);
    const faceH = groundF - topF;
    ctx.fillStyle = COLORS.buildingFront;
    ctx.fillRect(faceL, topF, faceW, faceH);

    // Lit windows — grid counts come from INTRINSIC world size (not screen size),
    // so each building's pattern is fixed and never re-tiles as it approaches.
    if (b.relZ < BUILDING_WINDOW_NEAR_WZ && faceH > 10) {
      const rows    = Math.max(2, Math.min(14, Math.round(b.height / 0.16)));   // shared by both faces
      const winFade = fade * (1 - b.relZ / BUILDING_WINDOW_NEAR_WZ);            // fade in as it nears
      ctx.fillStyle   = COLORS.buildingWindow;
      ctx.globalAlpha = winFade;

      // Front face — axis-aligned cells
      if (faceW > 6) {
        const cols = Math.max(1, Math.min(6, Math.round(b.width / 32)));
        const cw = faceW / cols, ch = faceH / rows, mx = cw * 0.30, my = ch * 0.32;
        for (let r = 0; r < rows; r++)
          for (let c = 0; c < cols; c++) {
            if (buildingHash(b.k * 131 + r * 13 + c, b.saltBase + 5) < 0.55) continue;   // ~45% lit
            ctx.fillRect(faceL + c * cw + mx, topF + r * ch + my, cw - 2 * mx, ch - 2 * my);
          }
      }

      // Inner side face — perspective cells across the box depth (front→back).
      // Bilinear map: u runs along depth (front edge→back edge), v up the height.
      if (Math.abs(fInner - bInner) > 4) {
        const sCols = 2 + Math.floor(buildingHash(b.k, b.saltBase + 6) * 2);   // 2–3, stable per building
        const mu = 0.22, mv = 0.32;
        const px = (u)    => fInner + (bInner - fInner) * u;
        const py = (u, v) => {
          const yb = groundF + (groundB - groundF) * u;   // bottom edge at depth u
          const yt = topF    + (topB    - topF)    * u;   // top edge at depth u
          return yb + (yt - yb) * v;
        };
        for (let r = 0; r < rows; r++)
          for (let c = 0; c < sCols; c++) {
            if (buildingHash(b.k * 131 + r * 13 + c, b.saltBase + 7) < 0.55) continue;   // independent pattern
    ...
    // Crisp silhouette outline against the gutter (front + visible side/top edges).
    ...
    ctx.lineTo(bOuter, topB);                                       // roof ridge
    ctx.moveTo(fOuter, topF);   ctx.lineTo(fOuter, groundF);        // outer vertical edge
    ctx.moveTo(bInner, groundB); ctx.lineTo(bInner, topB);          // back inner vertical
    ctx.stroke();

    // Neon rim on the near vertical corner (the lit edge toward the road).
    ctx.strokeStyle = COLORS.buildingRim;
    ctx.lineWidth   = Math.max(1, 2 / relZf);
    ctx.shadowColor = COLORS.buildingRim;
...
/** Deterministic pseudo-random value in [0,1) for skyline slot k (stable per slot/salt). */
...
function applyGateSection(sec) {
  ...
  if (Game.state === 'running') {   // stay silent during the title-screen attract demo
...
function startGame() {
  ...
  hideTitle();
  hideLeaderboard();
  startMusic();   // music only plays during a real run
}
...
function startTitleDemo() {
  ...
  showTitle();
  hideLeaderboard();
  fadeOutMusic();   // fade the run's music to silence over MUSIC_FADE_MS (no-op if not playing)
}
...
// ═══════════════════════════════════════════════════════════════════
//  AUDIO
//  One looping BGM + a bank of one-shot SFX, keyed by event id.  Files
//  live under assets/.  ids with no matching file (e.g. 'fall',
//  'gameover') simply no-op, so call sites need no guards.
// ═══════════════════════════════════════════════════════════════════
const Sound = {
  bgm:     null,   // looping background music element
  buffers: {},     // id → template HTMLAudioElement (cloned per play so SFX can overlap)
  fadeRAF: null,   // active music-fade animation handle
};
...
function initAudio() {
  ...
  const files = {
    clash:        'assets/sfx/Fight_Explosion.mp3',   // rival combat burst
    comet:        'assets/sfx/Comet_Sound.mp3',       // comet streaks across the sky
    button:       'assets/sfx/Button_Click.mp3',      // UI button press
    gatePositive: 'assets/sfx/Gate_Positive.mp3',     // gate that grows the crowd
    gateNegative: 'assets/sfx/Gate_Negative.mp3',     // gate that shrinks the crowd
  };
...
/** Fire a one-shot sound effect by event id. Unknown ids are silently ignored. */
function playSound(id) {
  const base = Sound.buffers[id];
  if (!base) return;
  const a = base.cloneNode();   // clone so rapid/overlapping events don't cut each other off
  a.volume = SFX_VOLUME;
  a.play().catch(() => {});      // ignore autoplay-policy rejections before first interaction
}

/** Start (or resume) the looping background music — called when a run begins. */
function startMusic() {
  const bgm = Sound.bgm;
  if (!bgm) return;
  cancelMusicFade();
...
/** Fade the music to silence over MUSIC_FADE_MS, then stop it — on return to title. */
function fadeOutMusic() {
  ...
  const step = (now) => {
    ...
    if (t < 1) {
      ...
      bgm.pause();
      bgm.currentTime = 0;
      bgm.volume = BGM_VOLUME;   // primed for next run
      Sound.fadeRAF = null;
    }
  };
  Sound.fadeRAF = requestAnimationFrame(step);
...
function resolveRivalCombat(rival) {
  ...
  if (Game.state !== 'title') playSound('clash');   // keep the title attract screen quiet
}
...
function frame(timestamp) {
  ...
  drawBuildings(ctx);    // scrolling city skyline in the gutter (behind glow & gameplay)
  drawHorizonGlow(ctx);
